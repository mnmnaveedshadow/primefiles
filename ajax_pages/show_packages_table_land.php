            ...
            $ctid = $row['ctype_id'];
              ...
              <tr>
                ...
                <td> <?= getDataBack($conn,'tbl_package_type','pt_id',$ptId,'pt_name'); ?> </td>
                  ...
                  <td><?= $row['p_weight'] ?></td>
                  <td><a onclick="del_p_desktopLand(<?= $row['p_id'] ?>)" class="btn btn-danger btn-sm"> X </a></td>
              </tr>
          <?php }
          $charge_weight = max($tot_weight, customRound($tot_vol));
      } ?>
...
<br><br>
		<button type="button" name="button" class="btn btn-success" onclick="shippingSubmit(3,6)">Next</button>
